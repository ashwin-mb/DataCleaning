...
def attr_cumulative(T,field,by_field='bogus_flag',uniform=True,same_figure=False,sum_to_one=True,threaded=False,show_coords=True):
    if (not threaded):
...
def attr_cumulative_plot(T,field,by_field='bogus_flag',uniform=True,same_figure=False,sum_to_one=True,show_coords=True,x_ticks=15,y_ticks=10):
    """
...
def predictions2betas_curve(predictions,model_score_col='bogus_flag_Model7',label_col='bogus_flag',return_betas=True,plot_betas=False,axes=None,**kwargs):
    """
    predictions has a prediction and a label for each row (presumably it's one row per firm)
    if @return_betas then return the betas DF and not the axes object
    **kwargs is for the plot function
    """
    betas = predictions.sort_values(model_score_col)[label_col].cumsum() \
            .to_frame(name='bogus_cumul')
    betas['legit_cumul'] = arange(1,len(betas)+1)-betas['bogus_cumul']
    total_bogus = betas['bogus_cumul'].iloc[-1]
    total_legit = betas['legit_cumul'].iloc[-1]
    betas['beta_legit'] = betas['bogus_cumul'] /1./ total_bogus
    betas['beta_bogus'] = (total_legit - betas['legit_cumul']) /1./ total_legit
    if plot_betas:
        if axes is None:
            axes = plt.subplot(1,1,1)
        kw = dict(kwargs)
        if 'fontsize' in kwargs:
            fontsize = kw.pop('fontsize')
        else:
            fontsize = None
        axes.plot(betas['beta_bogus'],betas['beta_legit'],**kw)
        axes.tick_params(labelsize=fontsize)
        axes.set_ylabel('beta legit (% of bogus missed)',fontsize=fontsize)
        axes.set_xlabel('beta bogus (% of legit insulted)',fontsize=fontsize)
        # betas.plot('beta_bogus','beta_legit')
    if return_betas:
        return betas
    return axes

def attr_scatter_1d(T,field1,by_field='bogus_flag',uniform=True,smearY=0.1,smearX=0.01):
    """
...
def attr_scatter(T,field1,field2=None,by_field='bogus_flag',uniformX=False,uniformY=False,smearX=0,smearY=0,uniform=None):
