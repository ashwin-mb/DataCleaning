...
ef(r'D:\Ashwin\do\init_sm_old.py')
...
def do_all():
    ...
    femq12 = pd.read_csv(r"H:\Ashwin_old\BogusFirmCatching_minus_ward\output_data\All_return_features_minus_q12_old.csv"); femq12['fold'] = (femq12['TIN_hash_byte']/32).astype(int)
    femq12 = femq12.drop(['Ward'], axis = 1 )
    ...
    # example 
    #removed ward from dealer_features if you drop Ward column
    firm_period_predictions,cv_model = cv_predictions_each_tax_period(ffemq12)
    firm_period_predictions.to_csv(r"H:\Ashwin_old\BogusFirmCatching_minus_ward\output_data\firm_period_predictions20190331.csv",index=False)
    ...
    # save old trained model location
    #h2o.save_model(cv_model, "H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions") #rf_cv_all_folds_20190401
...
def read_sub_returns():
    ...
    sr = pd.read_csv(r"H:\Ashwin_old\BogusFirmCatching_minus_ward\data\returns\sub_returns_no_strings.csv")
    sr = sr[ sr['Year']>=2013 ]
    return sr
...
def prepare_h2o_frame():
    fr = h2o.import_file(path=r"H:\Ashwin_old\BogusFirmCatching_minus_ward\data\returns\sub_returns_no_strings.csv") # r"Z:\sub_returns.csv"
    fr = fr[fr['Year'] >=2013]
...
def load_feature_label_table(save_as_csv=False,femq12=None):
    """
    ultimate source of data is H:\Ashwin_old\BogusFirmCatching_minus_ward\data\PreliminaryAnalysis\BogusDealers\<dta & csv files>
    see also funcs from Shekhar's files:
    load_*() in H:\Ashwin_old\shekhar_code_github\BogusFirmCatching_minus_ward\init_sm.py
    load_everything() in H:\Ashwin_old\BogusFirmCatching_minus_ward\ml_funcs.py
    """
    if femq12 is None:
...
def explore_classification(label_var='bogus_flag',create_plots=False):
    ...
    if create_plots:
        ...
        plot=compare_models(rf_models,legends, of=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Graphs\{label}_comparison_plot_AllCombinations_minusq12_numericmerge_withds.html'.format(label=label_var),\
                            title='Comparing All Models, {label}'.format(label=label_var))
        show(plot)

        for i in xrange(len(rf_models)):
            h2o.save_model(rf_models[i],path=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\diff_feature_sets\20190331')

        for i in xrange(7):
            show(analyze_model(rf_models[i],of=r"H:\Ashwin_old\BogusFirmCatching_minus_ward\Graphs\{}_model{}_v2_numericmerge_withds.html".format(label_var,i+1),n_rows=30))
...
def load_models(model_dir_path=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\diff_feature_sets\20190331'):
    models = map(h2o.load_model,glob.glob(os.path.join(model_dir_path,'*')))
    return models

def explore_multiple_time_periods(predictions_file=r"H:\Ashwin_old\data\PreliminaryAnalysis\BogusDealers\Predictions_bogus_online_v2_numericmerge_withDS.csv",
...
def cv_predictions_each_tax_period(feature_table,label_var='bogus_flag',k_folds=8,feature_set=None,model_save_dir=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions',save_predictions_as_csv=False,model=None):
    """
    ...
    if save_predictions_as_csv:
        ...
        # H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions\20190331\

    return firm_period_predictions,model

# def cv_predictions_each_tax_period_k_models(feature_table,label_var='bogus_flag',k_folds=8,feature_set=None,model_save_dir=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions\20190331',save_predictions_as_csv=False):
#     """
#     create predictions for the entire dataset (each row is firm X quarter) using cross-validation
#     returns a (DataFrame of firm-period predictions, list of models)
...
def point_in_time_simulations():
    ...
    last_appearance = femq12.groupby('Mtin')['TaxQuarter'].max().reset_index()
    femq12 = femq12.merge(last_appearance.rename(columns={'TaxQuarter':'DealerLastTaxQuarter'}),on='Mtin')
    # firm_period_predictions,cv_model = cv_predictions_each_tax_period(ffemq12)
    # avg_predictions = firm_period_predictions.groupby('Mtin').mean()
    assessment_list = []
    step = 2
    for t in xrange(9+1,20+1,step):
        ...
        point_in_time_femq12.loc[point_in_time_femq12['DealerLastTaxQuarter']>=t,'bogus_flag'] = 0
        point_in_time_h2o = h2o.H2OFrame(point_in_time_femq12.drop('DealerLastTaxQuarter',axis=1))
        print time.ctime()
        save_dir = r"H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions\20190331\point_in_time_q{:02d}".format(t)
        try:
    ...
    for criterion in ['performance','revenue','performance_revenue_optimized','revenue_optimized']:
        ...
        assessments[criterion].to_csv('H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions\20190331\point_in_time_{}.csv'.format(criterion),index=False)
    analyze_performances(assessments['performance'])

def get_future_predictions(firm_period_predictions,t,femq12,model_score_col='model_score_bogus_flag',label_var='bogus_flag'):
    dealer_predictions = firm_period_predictions[['Mtin',model_score_col]].groupby('Mtin').mean().reset_index()
    # unblinding - real classes
    dealer_classes = femq12[['Mtin','bogus_flag','DealerLastTaxQuarter']].drop_duplicates(subset=['Mtin'])
    dealer_future_tax = femq12.query('TaxQuarter>@t').groupby('Mtin')['TaxCreditBeforeAdjustment'].sum().reset_index()
...
def point_in_time_assess_performance(firm_period_predictions,t,femq12,model_score_col='model_score_bogus_flag',label_var='bogus_flag',save_dir=None):
    """
...
def analyze_performances(performances,fontsize=16):
    ...
    lifespan = femq12.query('bogus_flag==1').groupby('Mtin')['TaxQuarter'].agg(['min','max'])
...
def stats_for_paper():
    ...
    femq12.groupby('bogus_flag')['TaxCreditBeforeAdjustment'].agg(['mean','sum'])
    print 'number of firms:'
    femq12.groupby('bogus_flag')['Mtin'].nunique()

    # all time CV performance
    firm_period_predictions_alltime = pd.read_csv(r"H:\Ashwin_old\BogusFirmCatching_minus_ward\output_data\firm_period_predictions20190331.csv")
    ...
    print perf
    perf.to_csv(r"H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\cv_predictions\20190331\alltime_cv_avg_performance.csv")
...
def different_feature_sets_save_predictions(ffemq12):
    for i,feature_set in enumerate(FEATURE_SETS):
        ...
        print time.ctime()
        try:
            model_save_dir=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\diff_feature_sets\20190331\feature_set_{}'.format(i)
            ...
            firm_period_predictions.to_csv(os.path.join(model_save_dir,"firm_period_predictions20190331.csv"),index=False)
            avg_predictions = firm_period_predictions.groupby('Mtin').mean()
            performance_on_top_recommendations(avg_predictions,model_score_col,label_var,1,plot=False).to_csv(os.path.join(model_save_dir,"avg_predictions_performance_20190331.csv"))
        except Exception as exc:
...
def different_feature_sets_betas_plot():
    ...
    for i in lrange(FEATURE_SETS):
        print i,
        model_save_dir=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\diff_feature_sets\20190331\feature_set_{}'.format(i)
        firm_period_predictions = pd.read_csv(os.path.join(model_save_dir,"firm_period_predictions20190331.csv"))
        avg_predictions = firm_period_predictions.groupby('Mtin').mean()
        betas = predictions2betas_curve(avg_predictions,model_score_col,label_var,return_betas=True,plot_betas=False)
        betas.to_csv(os.path.join(model_save_dir,"betas20190331.csv"))
...
def different_classifiers_save_predictions(ffemq12,model_score_col='model_score_bogus_flag',label_var='bogus_flag'):
    ...
    for algo_name,model in CLASSIFIERS.items():
        ...
        print time.ctime()
        try:
            model_save_dir=r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\diff_classifiers\{}\classifier_{}'.format(CLASSIFIER_DATE_STR,algo_name)
            if not os.path.exists(model_save_dir):
                os.mkdir(model_save_dir)
...
def different_classifiers_performance_plots_files(model_score_col='model_score_bogus_flag',label_var='bogus_flag'):
    ...
    """
    parent_dir = r'H:\Ashwin_old\BogusFirmCatching_minus_ward\Models\diff_classifiers\{}'.format(CLASSIFIER_DATE_STR)
    axes = None
    all_top_perfs = []
    for algo_name,model in CLASSIFIERS.items():
        print algo_name
    ...
    plt.subplots_adjust(bottom=0.15)
    # plt.savefig(r"H:\Ashwin_old\BogusFirmCatching_minus_ward\figures\models\different_classifiers_performance_by_inspection_group_bars.png")
