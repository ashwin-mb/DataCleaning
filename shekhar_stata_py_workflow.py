...
ef(r'D:\Ofir\init.py')
ef(r'D:\shekhar_code_github\BogusFirmCatching\init_sm.py')
ef(r'D:\shekhar_code_github\BogusFirmCatching\ml_funcs.py')
...
def do_all():
    ...
    femq12 = pd.read_csv(r"D:\Ofir\output_data\all_returns_features_minus_q12.csv"); femq12['fold'] = (femq12['TIN_hash_byte']/32).astype(int)
    ...
    # firm_period_predictions.to_csv(r"D:\Ofir\output_data\firm_period_predictions20171225.csv",index=False)
...
def load_models(model_dir_path=r'D:\shekhar_code_github\BogusFirmCatching\Models\diff_feature_sets\20170515'):
    models = map(h2o.load_model,glob.glob(os.path.join(model_dir_path,'*')))
    return models

def explore_multiple_time_periods(predictions_file=r"D:\data\PreliminaryAnalysis\BogusDealers\Predictions_bogus_online_v2_numericmerge_withDS.csv",
                                  label_var='bogus_online',plot=False):
    predictions = pd.read_csv(predictions_file)
    predictions.sort_values(['DealerTIN','TaxQuarter'],inplace=True)
    keep_columns = [u'DealerTIN', u'TaxQuarter', u'bogus_online',
       u'bogus_cancellation',u'bogus_online_Model1', u'bogus_online_Model2',
       u'bogus_online_Model3', u'bogus_online_Model4', u'bogus_online_Model5',
       u'bogus_online_Model6', u'bogus_online_Model7']

    model_score_col = u'bogus_online_Model7'
...
def explore_multiple_aggregations(firm_period_predictions,model_score_col,label_var='bogus_online',plot=True,keep_columns=None):
    """
    ...
    n_tax_quarters = 11
    grouped_predictions = firm_period_predictions[keep_columns].groupby('DealerTIN')
    pred_dfs = {}
    ...
    # pred_dfs['pred_geo_odds']['bogus_online'] = pred_dfs['pred_first']['bogus_online']
    if plot:
        plt.figure()
...
def explore_multiple_time_period_aggregations_point_in_time(firm_period_predictions,model_score_col='model_score_bogus_online',label_var='bogus_online',plot=True):
    grouped_predictions = firm_period_predictions.groupby('DealerTIN')
    last_appearance = grouped_predictions['TaxQuarter'].max().reset_index()
    # pd.crosstab(last_appearance['TaxQuarter'],last_appearance['bogus_online'])
    for t in xrange(9,21):
        still_operating_tins = set(last_appearance.query('TaxQuarter>=@t')['DealerTIN'])
        past_predictions = firm_period_predictions.query('(TaxQuarter<=@t) and DealerTIN in @still_operating_tins')
        # grouped_predictions = past_predictions.groupby('DealerTIN')
...
def performance_on_top_recommendations(pred,model_score_col='bogus_online_Model7',label_var='bogus_online',scaling_factor=8,plot=False):
    """
...
def cv_predictions_each_tax_period(feature_table,label_var='bogus_online',k_folds=8,feature_set=None,model_save_dir=r'D:\shekhar_code_github\BogusFirmCatching\Models\cv_predictions',save_predictions_as_csv=False,model=None):
    """
    ...
    feature_table_cols = ['DealerTIN','TaxQuarter','TIN_hash_byte','fold',label_var]
    try:
    ...
    if save_predictions_as_csv:
        ...
        # D:\shekhar_code_github\BogusFirmCatching\Models\cv_predictions\20170524\

    return firm_period_predictions,model

# def cv_predictions_each_tax_period_k_models(feature_table,label_var='bogus_online',k_folds=8,feature_set=None,model_save_dir=r'D:\shekhar_code_github\BogusFirmCatching\Models\cv_predictions\20170524',save_predictions_as_csv=False):
#     """
#     create predictions for the entire dataset (each row is firm X quarter) using cross-validation
#     returns a (DataFrame of firm-period predictions, list of models)
...
#         predictions_and_ids = pd.concat( [prediction_fold[['DealerTIN','TaxQuarter','TIN_hash_byte',label_var]].as_data_frame(use_pandas=True), predictions],axis=1,ignore_index=True)
...
def aggregate_predictions_multiple_time_periods(firm_period_predictions,model_score_col='model_score_bogus_online',at_t=None,aggfunc=np.mean):
    """
    ...
    grouped_predictions = firm_period_predictions.groupby('DealerTIN')
    last_appearance = grouped_predictions['TaxQuarter'].max().reset_index()
    still_operating_tins = set(last_appearance.query('TaxQuarter>=@at_t')['DealerTIN'])
    # TODO: this seems wrong - we use the future predictions for TINs still operating.
    past_predictions = firm_period_predictions.query('DealerTIN in @still_operating_tins')
    grouped_predictions = past_predictions.groupby('DealerTIN')
    agg_predictions = grouped_predictions.agg(aggfunc).reset_index()

    return agg_predictions

def get_suspicious(firm_period_predictions,model_score_col='model_score_bogus_online',at_t=None):
    if at_t is None:
        exactly_at_t = False
        at_t = firm_period_predictions['TaxQuarter'].max()
    else:
    ...
    grouped_predictions = firm_period_predictions.query('TaxQuarter<=@at_t').groupby('DealerTIN')
    ...
    predictions['bogus_online'] = grouped_predictions['bogus_online'].first()
    if exactly_at_t:
        predictions = predictions.query('last_quarter==@at_t')
    return predictions
    # predictions['last_quarter'].value_counts()

# mean_aggregated = firm_period_predictions.groupby('DealerTIN')[['model_score_bogus_online','bogus_online']].mean()
def check_model_calibration(predictions,model_score_col='model_score_bogus_online',label=None):
    pred_bin = pd.cut(predictions[model_score_col],[1,0.5,0.1,0.01,0.001,0.0001,0][::-1])
    gr = predictions.groupby(pred_bin)
    red = gr['bogus_online'].agg({'label_mean':'mean','label_sum':'sum','n':len})
...
def point_in_time_simulations():
    femq12['fold'] = (femq12['TIN_hash_byte']/32).astype(int) # h2o.as_list(ffemq12['fold'])['fold']
    last_appearance = femq12.groupby('DealerTIN')['TaxQuarter'].max().reset_index()
    femq12 = femq12.merge(last_appearance.rename(columns={'TaxQuarter':'DealerLastTaxQuarter'}),on='DealerTIN')
    # firm_period_predictions,cv_model = cv_predictions_each_tax_period(ffemq12)
    # avg_predictions = firm_period_predictions.groupby('DealerTIN').mean()
    assessment_list = []
    step = 2
    for t in xrange(9+1,20+1,step):
        print t
        print time.ctime()
        print '-'*30
        ...
        point_in_time_femq12 = femq12.query('TaxQuarter<=@t').copy()
        # bogus known by time t, so all those canceled after seem legit
        point_in_time_femq12.loc[point_in_time_femq12['DealerLastTaxQuarter']>=t,'bogus_online'] = 0
        point_in_time_h2o = h2o.H2OFrame(point_in_time_femq12.drop('DealerLastTaxQuarter',axis=1))
        print time.ctime()
        save_dir = r"D:\shekhar_code_github\BogusFirmCatching\Models\cv_predictions\20171226\point_in_time_q{:02d}".format(t)
        try:
            ...
            assessment = point_in_time_assess_performance(firm_period_predictions_t,t,femq12,save_dir=save_dir)
    ...
    for criterion in ['performance','revenue','performance_revenue_optimized','revenue_optimized']:
        ...
        assessments[criterion].to_csv('D:\\shekhar_code_github\\BogusFirmCatching\\Models\\cv_predictions\\20171226\\point_in_time_{}.csv'.format(criterion),index=False)
    analyze_performances(assessments['performance'])

def get_future_predictions(firm_period_predictions,t,femq12,model_score_col='model_score_bogus_online',label_var='bogus_online'):
    dealer_predictions = firm_period_predictions[['DealerTIN',model_score_col]].groupby('DealerTIN').mean().reset_index()
    # unblinding - real classes
    dealer_classes = femq12[['DealerTIN','bogus_online','DealerLastTaxQuarter']].drop_duplicates(subset=['DealerTIN'])
    dealer_future_tax = femq12.query('TaxQuarter>@t').groupby('DealerTIN')['TaxCreditBeforeAdjustment'].sum().reset_index()
    dealer_classes_tax = dealer_classes.merge(dealer_future_tax,how='left',on='DealerTIN').fillna(0)
    # dealer_predictions_classes = dealer_predictions.merge(dealer_classes,how='left',on='DealerTIN')
    dealer_predictions_classes_tax = dealer_predictions.merge(dealer_classes_tax,how='left',on='DealerTIN')
...
def point_in_time_assess_performance(firm_period_predictions,t,femq12,model_score_col='model_score_bogus_online',label_var='bogus_online',save_dir=None):
    """
    ...
    """
    future_predictions = get_future_predictions(firm_period_predictions,t,femq12,model_score_col,label_var)
    results = {}
...
def analyze_performances(performances,fontsize=16):
    ...
    lifespan = femq12.query('bogus_online==1').groupby('DealerTIN')['TaxQuarter'].agg(['min','max'])
    [((lifespan['min']<=t) & (t<=lifespan['max'])).sum() for t in xrange(10,21,2)]
    return axes

def stats_for_paper():
    # share to unregistered > 0 is predictive
    qq = femq12[['PercPurchaseUnregisteredDealer','bogus_online']].copy()
    qq['share_to_unregistered'] = pd.cut(qq['PercPurchaseUnregisteredDealer'],[-inf,0,1])
    qq.groupby('share_to_unregistered')['bogus_online'].agg(['sum','size','mean'])

    # all revenue lost to bogus firms
    print 'TaxCreditBeforeAdjustment:'
    femq12.groupby('bogus_online')['TaxCreditBeforeAdjustment'].agg(['mean','sum'])
    print 'number of firms:'
    femq12.groupby('bogus_online')['DealerTIN'].nunique()

    # all time CV performance
    firm_period_predictions_alltime = pd.read_csv(r"D:\Ofir\output_data\firm_period_predictions20171225.csv")
    avg_predictions = firm_period_predictions_alltime.groupby('DealerTIN')[['bogus_online','model_score_bogus_online']].mean()
    perf = performance_on_top_recommendations(avg_predictions,model_score_col,label_var,1,plot=False)
    print perf
    perf.to_csv(r"D:\shekhar_code_github\BogusFirmCatching\Models\cv_predictions\20171226\alltime_cv_avg_performance.csv")
...
def plot_success_on_top_predictions_continuous(agg_predictions,top_n_predictions=400,label_var='bogus_online',model_score_col='model_score_bogus_online',axes=None,label='',save_path=None):
    """
...
def different_feature_sets_save_predictions(ffemq12):
    for i,feature_set in enumerate(FEATURE_SETS):
        ...
        print time.ctime()
        try:
            model_save_dir=r'D:\shekhar_code_github\BogusFirmCatching\Models\diff_feature_sets\20171231\feature_set_{}'.format(i)
            ...
            firm_period_predictions.to_csv(os.path.join(model_save_dir,"firm_period_predictions20171228.csv"),index=False)
            avg_predictions = firm_period_predictions.groupby('DealerTIN').mean()
            performance_on_top_recommendations(avg_predictions,model_score_col,label_var,1,plot=False).to_csv(os.path.join(model_save_dir,"avg_predictions_performance_20171228.csv"))
        except Exception as exc:
...
def different_classifiers_performance_plots_files(model_score_col='model_score_bogus_online',label_var='bogus_online'):
    """
    Load CV predictions of different classification algorithms, calculate betas, save and plot curves.
    """
    parent_dir = r'D:\shekhar_code_github\BogusFirmCatching\Models\diff_classifiers\{}'.format(CLASSIFIER_DATE_STR)
    axes = None
    all_top_perfs = []
    for algo_name,model in CLASSIFIERS.items():
        print algo_name
