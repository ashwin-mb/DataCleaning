# -*- coding: utf-8 -*-
"""
Created on Mon Jan 21 09:26:26 2019
...
### C1: Difference between 1st and 3rd line
bogus_dealers_dir = r"H:\Ashwin\dta\features"
if socket.gethostname() == 'WIN-0HSQUR8Q0D3':
    bogus_dealers_dir = r"H:\Ashwin\dta\features"

### C2: Need to create TIN_hash_byte
def load_returns():
    returns = pd.read_stata(bogus_dealers_dir+"\FeatureReturns.dta", convert_categoricals=False)
    returns['TIN_hash_byte']=returns['Mtin'].astype(str).apply(lambda x: ord(md5(x).digest()[0]))
...
def load_networkfeaturessales(TaxQuarter=9):
    NetworkFeaturesSales=pd.read_csv(bogus_dealers_dir+'\\bogusdealersNetworkFeaturesSales'+str(TaxQuarter)+'.csv')
#    NetworkFeaturesSales=NetworkFeaturesSales.dropna(subset=['bogus_online'])
    ...
    NetworkFeaturesSales= NetworkFeaturesSales.rename(index=str, columns={"__id":"Mtin","component_id":"Sales_component_id",\
...
def load_networkfeaturespurchases(TaxQuarter=9):
    NetworkFeaturesPurchases=pd.read_csv(bogus_dealers_dir+'\\bogusdealersNetworkFeaturesPurchases'+str(TaxQuarter)+'.csv')
#    NetworkFeaturesPurchases=NetworkFeaturesPurchases.dropna(subset=['bogus_online'])
    ...
    NetworkFeaturesPurchases= NetworkFeaturesPurchases.rename(index=str, columns={"__id":"Mtin","component_id": "Purchases_component_id","pagerank":"Purchases_pagerank","triangle_count":"Purchases_triangle_count","in_degree":"Purchases_in_degree","out_degree":"Purchases_out_degree","core_id":"Purchases_core_id"})
...
def load_h2odataframe_returns(fr):
    """
...
##### C3: No column with names bogus_online, bogus_any;
def set_return_factors(fr):
    ...
    fr['bogus_flag'] = fr['bogus_flag'].asfactor()  
